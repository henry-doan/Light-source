...
var MAX_NODES = 500;
var ANIMATE_AVERAGE = 1;

var BACKGROUND_LIGHT = 0.05;
var lightVector;
...
var toolbarHeight = 0;
var toolbarX = width * 0.01 - translateX;
var toolbarY = width * 0.01 - translateY;
...
/********************************************************
 *      Light
 * A light is a position in space on a sphere
 * with radius 1, defined by two angles,
 * which converts to a vector pointing to the
 * center of the screen
*********************************************************/

var Light = function(theta, phi) {
    this.theta = theta;
    this.phi = phi;
    this.update();
};
...
Light.prototype.update = function() {
    this.x = sin(this.theta) * cos(this.phi);
    this.y = sin(this.theta) * sin(this.phi);
...
Face.prototype.update = function() {
    ...
    var l = 1 - constrain(dotProduct(lightVector, normal), BACKGROUND_LIGHT, 1);
    var p = 0.01;
    
    if (l < p) {
        l = max(0, 0.4 * l / p);
        this.light = lerpColor(this.color, WHITE, l);
    } else {
...
var createPlane = function(x, y, z, w, h) {
    var w2 = w / 2;
    var h2 = h / 2;
...
lightVector = new Light(180, 0);

var d = 240;
var wirefame = createCuboid(-d / 2, -d / 2, -d / 2, d, d, d);
var myShape = new Wireframe(wirefame);
var grid = new Grid(3, 100, 120);
...
var keyboardRotateShape = function() {
    if (keyCode === LEFT) {
